...
def draw_banner():
 ...
 print(f"======================================{RESET}")
 print(f"{CYAN}[1]{RESET} Search & Save New Article")
 print(f"{CYAN}[2]{RESET} Open & Read Local Saved Files")
 print(f"{CYAN}[3]{RESET} Vault File Manager (Rename/Delete)")
 print(f"{CYAN}[4]{RESET} Chat with AI Over Saved Articles")
 print(f"{CYAN}[5]{RESET} Exit System")
 print(f"{CYAN}======================================{RESET}")
...
while True:
 ...
 if choice == '1':
  ...
  if not ai_summary:
   ai_summary = "Ollama model offline or background server not running. (To turn on AI, open a new Termux tab and run 'ollama run llama3.2:1b')."
  ...
  try:
   ...
   if 1 <= read_choice <= len(files):
    ...
    os.system('clear')
    print(f"{CYAN}{BOLD}======================================")
    print(f"📖 READING: {target_file.replace('_', ' ').replace('.txt', '')}")
    print(f"{YELLOW}📊 STATS: {words} Words | {chars} Characters{RESET}")
  ...
  files = [f for f in os.listdir('.') if f.endswith('.txt') and not f.startswith('.')]
  if not files:
   print(f"{RED}No files available to manage.{RESET}")
  ...
  try:
   ...
   if 1 <= manage_choice <= len(files):
    ...
    print(f" {RED}{RESET} Delete File")
    print(f" {YELLOW}{RESET} Rename File")
  ...
  files = [f for f in os.listdir('.') if f.endswith('.txt') and not f.startswith('.')]
  if not files:
   print(f"{RED}Error: Requires saved text articles in your vault folder.{RESET}")
   input(f"\nPress Enter to return...")
   continue
  for idx, f_name in enumerate(files, 1): print(f"{GREEN}[{idx}]{RESET} {f_name.replace('_', ' ').replace('.txt', '')}")
  try:
   ...
   if 1 <= chat_choice <= len(files):
    ...
    print(f"🤖 AI CONTEXT STREAM LOADED: {target_file.replace('.txt','')}")
    print(f" Type 'back' anytime to return to main menu.")
    print(f"{GREEN}{BOLD}======================================{RESET}\n")
    ...
    while True:
     ...
     if ai_response:
      ...
      print(f"{RED}Ollama background server offline. (Run 'ollama run llama3.2:1b' in another tab).{RESET}\n")
   else: print(f"{RED}Invalid assignment position.{RESET}")
